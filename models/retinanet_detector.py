...
class RetinaNetDetector(DetectorBase):
        ...
        *,
        ...
        if model is None:
            if init_weights_dir:
                ...
                state_path = os.path.join(init_weights_dir, "retinanet_model.pth")
    ...
    def save(self, path: str) -> None:
        ...
        torch.save(self.model.state_dict(), os.path.join(path, "retinanet_model.pth"))
        meta = {"num_classes": self.num_classes}
        with open(os.path.join(path, "retinanet_meta.json"), "w", encoding="utf-8") as handle:
            json.dump(meta, handle, indent=2, sort_keys=True)

    @classmethod
    def load(cls, path: str, device: torch.device):
        meta_path = os.path.join(path, "retinanet_meta.json")
        with open(meta_path, "r", encoding="utf-8") as handle:
            meta = json.load(handle)

        num_classes = int(meta["num_classes"])
        model = cls._build_model(num_classes, pretrained=False)
        state = torch.load(os.path.join(path, "retinanet_model.pth"), map_location=device)
        model.load_state_dict(state)
